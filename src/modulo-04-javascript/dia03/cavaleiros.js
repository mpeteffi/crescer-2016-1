...
$(function () {
  ...
  function adicionarImagens(){
    ...
    goldSaints.forEach(function(cavaleiro) {
      ...
      $('#cavaleiros img').mouseover(function() {
        projetarPropriedadesDoCavaleiro($(this).attr('id'));
      });
      // remover conteudo de propriedades ao retirar o mouse.
      $('#cavaleiros img').mouseout(function() {
        $('#dados').empty();
      });
      // remover cavaleiro do array
      $('#cavaleiros button').click(removeCavaleiro);
...
function formToCavaleiro($form){
  var formData = new FormData($form[0]);
  return {
    id: goldSaints[goldSaints.length-1].id +1 ,
    nome: formData.get('nome'),
    dataNascimento: formData.get('dataNascimento'),
    alturaCm: duasCasas(mToCm(formData.get('alturaM'))),
    pesoLb: duasCasas(kgToLb(formData.get('pesoKg'))),
